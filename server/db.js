...
try {
  db.exec(`ALTER TABLE users ADD COLUMN api_key TEXT NOT NULL DEFAULT ''`);
} catch (_) { /* 列已存在 */ }

// Seed admin 账号（仅当 users 表为空时）
const count = db.prepare('SELECT COUNT(*) as c FROM users').get().c;
if (count === 0) {
  const hash = bcrypt.hashSync('majiwei666', 10);
  db.prepare('INSERT INTO users (username, password, role, credits) VALUES (?, ?, ?, ?)').run('majiwei', hash, 'admin', 99999);
  console.log('[db] admin 账号已创建');
}
